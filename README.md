...
The current Phase 6 prototype supports a small interpreter for declarations, assignments, input, output, simple expressions, integer `DIV` and `MOD`, comparisons, boolean logic, `IF` / `ELSE` / `ENDIF` conditionals, `FOR` / `NEXT` loops, `WHILE` / `ENDWHILE` loops, and `REPEAT` / `UNTIL` loops.
...
Phase 6 frontend prototype. Supported syntax is intentionally small; arrays, CASE, functions, procedures, files, records, and OOP are not implemented yet.
